...
// updateSettings applies a partial update to the account's settings row;
// omitted fields keep their stored values. A request carrying a cluster
// bootstraps the row when the account doesn't have one yet.
func (h *handler) updateSettings(w http.ResponseWriter, r *http.Request) {
	...
	updated, err := h.manager.UpdateSettings(r.Context(), userAuth.AccountId, userAuth.UserId, types.SettingsUpdateFromAPIRequest(&req))
	if err != nil {
		util.WriteError(r.Context(), err, w)
		return
	}
