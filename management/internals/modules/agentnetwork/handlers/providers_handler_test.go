...
// for PUT /agent-network/providers/{id}: a request carrying only the required
// fields must leave every omitted optional field — extra_values,
// metadata_disabled, models, identity headers, enabled, skip_tls_verification
// — exactly as stored. Before the handler merged onto the existing row, such
// an update silently wiped dashboard-configured extra_values and reset the
// toggles account-wide.
func TestProviderHandler_UpdatePreservesOmittedFields(t *testing.T) {
	...
	require.Len(t, updated.Models, 1, "omitted models must preserve the stored list")
	assert.Equal(t, "gpt-4o", updated.Models[0].Id)
