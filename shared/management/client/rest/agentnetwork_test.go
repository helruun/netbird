...
func TestAgentNetwork_UpdateSettings_200(t *testing.T) {
	withMockClient(func(c *rest.Client, mux *http.ServeMux) {
		mux.HandleFunc("/api/agent-network/settings", func(w http.ResponseWriter, r *http.Request) {
			...
			require.NotNil(t, req.EnableLogCollection)
			assert.True(t, *req.EnableLogCollection)
			// Omitted fields must be absent so the server-side merge
			// preserves the account's current values.
			assert.NotContains(t, string(reqBytes), "redact_pii")
			assert.NotContains(t, string(reqBytes), "enable_prompt_collection")
			retBytes, _ := json.Marshal(testAgentNetworkSettings)
			_, err = w.Write(retBytes)
			require.NoError(t, err)
		})
